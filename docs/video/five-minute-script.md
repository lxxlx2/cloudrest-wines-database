...
| 1:10–1:40 | 1 | Demonstrate one valid integrity test and the PO Box trigger failure; explain database-level protection. |
| 1:40–2:05 | Rianna | Run Query 1 and state which areas need both training categories. |
| 2:05–2:30 | Rianna | Run Query 2 and explain exposure-adjusted incident rate. |
...
| 3:20–3:50 | 1 | Call `getExpiringQualifications(30)` and `(90)` and compare results. |
| 3:50–4:20 | Rianna | Run Query 6 and show the open-action priority output. |
| 4:20–4:40 | Rianna | Briefly show EXPLAIN: range scan on action status and `eq_ref` PK joins. |
